...
class Model(nn.Module):
    def __init__(self, classes):
        ...
        # Residual Block
        self.ResL1 = self.residual_block(inif, int(inif/2), 3, 1, 1)
        self.ResL2 = self.residual_block(inif*2, inif, 3, 1, 1)
        self.ResL3 = self.residual_block(inif*4, inif*2, 3, 1, 1)
        self.ResL4 = self.residual_block(inif*8, inif*4, 3, 1, 1)
        self.ResL5 = self.residual_block(inif*16, inif*8, 3, 1, 1)
    ...
    def dimension_inc(self, in_features, out_features, kernel_size, stride, padding, bias):
        Seq = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_features, out_features, kernel_size=kernel_size,
                      stride=stride, padding=padding, bias=bias),
        )
        return Seq

    def residual_block(self, in_features, mid_features, kernel_size, stride, padding):
        Seq = nn.Sequential(
            nn.BatchNorm2d(in_features),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Conv2d(mid_features, mid_features, kernel_size=kernel_size,
                      stride=stride, padding=padding, groups=30, bias=True),
            nn.BatchNorm2d(mid_features),
            nn.ReLU(),
            nn.Conv2d(mid_features, in_features, kernel_size=1,
                      stride=stride, padding=0, bias=True),
        )
        return Seq
